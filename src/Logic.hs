...
instance (Show r, Show t, Show c, Show o, Show q) => Show (Prop r t c o q) where
    show (Not p) = "Not " ++ show p
    show (Connected c p1 p2) = "Connected " ++ (show c) ++ (show p1) ++ (show p2)
    show (NonLogConnected c p1 p2) = "NonLogConnected " ++ (show c) ++ (show p1) ++ (show p2)
    show (Quantified c (Just d) p2) = "Quantified " ++ (show c) ++ "Just() unshowable"
    show (Quantified c (Nothing) p2) = "Quantified " ++ (show c) ++ "Nothing unshowable"
    show (Modal o p) = "Modal " ++ (show o) ++ (show p)
    show (Rel o p) = "Rel " ++ (show o) ++ (show p)
    show (Eet) = "Eet"
